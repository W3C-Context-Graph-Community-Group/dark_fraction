...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (action === 'emit') {
      ...
      if (schemas.has(url) && msg.data !== undefined) {
        const { valid, errors } = schemas.validate(url, msg.data);
        if (!valid) {
      ...
      const payload = JSON.stringify({ event: url, from: clientId, data: msg.data });
